...
export class ArtifactStore extends FileBackedRegistry<PersistedArtifactRecord> {
  ...
  async publish(input: PublishArtifactInput): Promise<PublishArtifactResult> {
    ...
    const committed = await this.mutateCache<CommittedPublish>(
      (records) => {
        ...
        const target = this.resolveTarget(records, input, callId, externalUrl, html !== null);
  ...
  private resolveTarget(
    ...
    // the same. Link-only on both sides, deliberately — a stored document's identity is its
    // bytes, and collapsing a publication into it would unlink content on the strength of a URL
    // scraped from tool-result text.
