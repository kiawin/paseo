...
const FILE_PATH = "/work/scratch/report.html";
...
function artifactToolCall(overrides: Partial<AgentTimelineItem> = {}): AgentTimelineItem {
  return {
    ...
    status: "completed",
    error: null,
    detail: { type: "artifact", url: URL_TEXT, title: "Q3 revenue", filePath: FILE_PATH },
    ...overrides,
    ...
    expect(published).toEqual([
      {
        ...
        url: URL_TEXT,
        title: "Q3 revenue",
        // Carried so capture can read the document itself; the row is all a replay has.
        filePath: FILE_PATH,
      },
    ]);
  });

  test("reports no file when the detail carries none", async () => {
    ...
    expect(published).toMatchObject([{ filePath: null, title: null }]);
  });
