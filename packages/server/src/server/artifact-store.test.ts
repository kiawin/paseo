...
describe("overwrite", () => {
  ...
  test("a publication with bytes to an already-listed URL stays separate", async () => {
    await store.publish({
      projectId: "prj_a",
      title: "Link",
      externalUrl: "https://claude.ai/code/artifact/abc",
    ...
    // URL text never unlinks stored content, so a stored publish is not collapsed into a link
    // row even when both name the same address.
