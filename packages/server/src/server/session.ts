...
export class Session {
      ...
      if (this.usesSelectiveTimelineDelivery() && serializedEvent.type === "attention_required") {
        ...
        this.emit({
          ...
          payload: this.buildAgentStreamPayload(
            event,
            this.artifactCompatibleStreamEvent(serializedEvent, this.supportsArtifactToolDetail()),
          ),
        });
      }
      return;
      ...
      this.onMessageToSource(source, {
        ...
        payload: this.buildAgentStreamPayload(
          ...
          this.artifactCompatibleStreamEvent(
            serializedEvent,
            capabilities.has(CLIENT_CAPS.artifactToolDetail),
          ),
        ),
      });
    }
  ...
  private forwardProviderSubagentUpdate(
    ...
    if (this.clientCapabilitiesBySource.size === 0 || !this.onMessageToSource) {
      if (
        ...
        this.emit(this.supportsArtifactToolDetail() ? message : legacyArtifactMessage);
      }
      return;
    }
    for (const [source, capabilities] of this.clientCapabilitiesBySource) {
      ...
      this.onMessageToSource(
        source,
        capabilities.has(CLIENT_CAPS.artifactToolDetail) ? message : legacyArtifactMessage,
      );
    }
  }
  ...
  private async handleArchiveWorkspaceRequest(
    ...
    try {
      ...
      // The record is archived whatever happened to the directory, so a refusal
      // has nowhere else to surface. Reported rather than only logged.
      this.emit({
        ...
        payload: {
          ...
          archivedAt,
          error: null,
          worktreeDirectoryRemoved: result.worktreeRemoval?.removed ?? null,
          worktreeRemovalRefusal: result.worktreeRemoval?.refusal ?? null,
        },
      });
    } catch (error) {
      ...
      this.emitForSource(
        {
          ...
          payload: {
            ...
            rows: rows.map((row) => ({
              item: supportsArtifactToolDetail ? row.item : downgradeArtifactToolDetail(row.item),
              timestamp: row.timestamp,
              seq: row.seq,
            })),
            error: null,
