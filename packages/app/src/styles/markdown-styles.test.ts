...
describe("markdownBlockIsRuleOnly", () => {
  it("recognises every thematic break spelling", () => {
    expect(markdownBlockIsRuleOnly("---")).toBe(true);
    expect(markdownBlockIsRuleOnly("***")).toBe(true);
    expect(markdownBlockIsRuleOnly("___")).toBe(true);
    expect(markdownBlockIsRuleOnly("- - -")).toBe(true);
    expect(markdownBlockIsRuleOnly("  ----  ")).toBe(true);
  });

  it("keeps a marker on any block that renders a line of text", () => {
    expect(markdownBlockIsRuleOnly("")).toBe(false);
    expect(markdownBlockIsRuleOnly("--")).toBe(false);
    // A setext heading and an hr followed by prose both have a line to sit on.
    expect(markdownBlockIsRuleOnly("Title\n---")).toBe(false);
    expect(markdownBlockIsRuleOnly("---\nstill prose")).toBe(false);
    // Four spaces make it a code block, which renders its own text.
    expect(markdownBlockIsRuleOnly("    ---")).toBe(false);
  });
});
