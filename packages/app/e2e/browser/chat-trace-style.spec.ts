...
test("renders a turn in the trace transcript style", async ({ page }) => {
  ...
  try {
    ...
    await assertPayloadCardTogglesOnPress(page);
    await assertShellStreamCopyButtons(page);
  } finally {
    await agent.cleanup();
  }
});
...
/**
 * The payload card opens and closes from its own content: the pill that says "Click to expand" is
 * `pointerEvents: none`, so every press here lands on the card under it.
 *
 * The card is resolved to a handle once, not held as a locator. A locator ending in `.first()`
 * re-resolves on every check, so the moment this card loses its pill the locator slides to the
 * next clamped card in the turn and every later assertion reads that one instead.
 */
async function assertPayloadCardTogglesOnPress(page: Page) {
  const card = await page
    ...
    .elementHandle();
  if (!card) {
    throw new Error("no clamped payload card in the turn");
  }
  const showsPill = () =>
    card.evaluate((node) => (node.textContent ?? "").includes("Click to expand"));
  const height = () => card.evaluate((node) => node.getBoundingClientRect().height);

  const collapsedHeight = await height();
  expect(collapsedHeight).toBeGreaterThan(0);

  await card.click({ position: { x: 12, y: 12 } });
  await expect.poll(showsPill).toBe(false);
  expect(await height()).toBeGreaterThan(collapsedHeight);

  // Pressing the open card puts it away, so the gesture that opened it is the one that closes it.
  // A press that ends a text selection is dropped instead; that guard is covered by
  // src/utils/text-selection.browser.test.ts, where a selection can be made deterministically.
  await card.click({ position: { x: 12, y: 12 } });
  await expect.poll(showsPill).toBe(true);
  expect(await height()).toBe(collapsedHeight);
}

/**
 * Each stream carries its own copy button in the label gutter, revealed on hover. Copying must
 * leave the card as it found it: the card itself toggles on press, and a copy that also collapsed
 * the thing you were copying from would be worse than no button.
 */
async function assertShellStreamCopyButtons(page: Page) {
  const inLabel = page.getByText("IN", { exact: true }).first();
  const shellCard = page.getByTestId("tool-call-detail").filter({ has: inLabel }).first();
  const copyButtons = shellCard.getByTestId("tool-call-shell-copy");
  await expect(copyButtons).toHaveCount(2);

  const card = await shellCard.elementHandle();
  if (!card) {
    throw new Error("no shell card in the turn");
  }
  const heightBefore = await card.evaluate((node) => node.getBoundingClientRect().height);

  // The button is transparent and takes no pointer events until its row is hovered, so the hover
  // has to land on the row first or the click has nothing to hit.
  await inLabel.hover();
  const copyCommand = copyButtons.first();
  await expect(copyCommand).toHaveAttribute("aria-label", "Copy command");
  await copyCommand.click();
  await expect(copyCommand).toHaveAttribute("aria-label", "Copied");

  expect(await card.evaluate((node) => node.getBoundingClientRect().height)).toBe(heightBefore);
}
