...
The preview never loads the companion link. It renders only the bytes the daemon stored; the link
opens as a normal browser navigation, either in a Paseo browser tab when the Agent links setting
is In Paseo or in the system browser otherwise.
