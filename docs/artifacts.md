...
A link-only publication resolves to its destination before minting a row: if the project already
lists a link-only artifact with the same `externalUrl`, the publish updates that record — new
title, refreshed `updatedAt`, the new caller's origin — instead of adding a second row for one
address. That is what keeps a Claude artifact the agent edits repeatedly to a single entry: every
edit is a new tool call, so the `(agentId, callId)` key in the previous section never matches, but
claude.ai publishes every version under the same URL. It is link-only on both sides by design — a
stored document's identity is its bytes, and a URL scraped from tool-result text never gets to
unlink them.
...
the live page stays canonical. The link follows the Agent links preference: In Paseo opens it in a
Paseo browser tab, while External browser uses the system browser. An agent that wants a document
Paseo actually holds — including Claude — writes the HTML and calls `publish_artifact` with it.
That path is unaffected and is the normal one.
