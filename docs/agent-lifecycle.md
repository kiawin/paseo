...
The two requests are for different moments. **Delete** in the workspace menu
archives and asks for the directory in one step; `workspace.worktree.remove` is
the retry afterwards, and refuses a workspace that is not archived yet, because
removing the directory under a live record leaves it pointing at nothing.

A refused removal is reported on the archive response
(`worktreeDirectoryRemoved`, `worktreeRemovalRefusal`), not only logged. The
workspace archives whichever way the removal went, so there is no later screen
that would notice, and a directory nobody knows survived is the failure worth
avoiding. Null means no removal was attempted — the caller did not ask, or the
workspace has no directory Paseo may delete.
